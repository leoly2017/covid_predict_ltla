               ...
               br(), column(width = 6, wellPanel( 
                 tagList(
                   ...
                   div(id="slider1", width = 6,
                       sliderInput("log", "Select week to predict",
                                   min=as.Date(week1), max=as.Date(week3),value=as.Date(week1), step = 7 ))),style = "padding: 10px 40px;")
               )),
             
             fluidRow(box(width = 12, br(), br(),
                          tags$p("View the predicted percentage in the number of COVID-19 cases for the selected week, compared with", tags$b(referenceweek), br(), "Use the", tags$b("search function on the right"), "for a specific local authority",style = "font-size:16px;"), 
             )),
             ...
             fluidRow(
               br(), column(width = 6, wellPanel( 
                 tagList(
                   ...
                   tags$style(type = 'text/css', '#slider1 .control-label {font-size: 20px}'),
                   div(id="slider1", width = 6,
                       sliderInput("log2", "Select week to predict",
                                   min=as.Date(week1), max=as.Date(week3),value=as.Date(week1), step = 7 ))),style = "padding: 10px 40px;")
               )),
...
server <- function(input, output, session) {
  ...
  output$image<-renderUI({
    if(input$log ==paste0(week1)){
      img(src =paste("images/logratemid2021-08-08.png"), width ="100%")
    ...
    if(input$log == paste0(week1)){
      DT::datatable(table1, caption = "Table",
                    ...
                    options =list(dom ='Bfrtip', buttons = c('copy','csv','excel'), 
                                  autoWidth= TRUE,scroller = TRUE, scrollX = TRUE, scrollY="700px", pageLength=50), rownames = FALSE)
    } else if (input$log == paste0(week2)){
  ...
  output$table_2 <- DT::renderDataTable({
    if(input$log2 == paste0(week1)){
      DT::datatable(table4, caption = "Table",
                    colnames= c( 'LTLA' ,"Point Estimate","95% CI lower","95% CI upper"),
                    options =list(pageLength=50, autoWidth= TRUE), rownames= FALSE)
